...
expression_matrix <- read.delim('/home/ec2-user/data/normal_counts.tsv', header = TRUE, row.names = 1, check.names = FALSE)
rownames(expression_matrix)=expression_matrix$gene_id
expression_matrix$gene_id = NULL
expression_matrix <- expression_matrix[grep("^ENSG", rownames(expression_matrix)), ]
...
write.table(aggregated_matrix, file = "ENCFF_normal_counts_converted.csv", sep = ",", quote = FALSE)
